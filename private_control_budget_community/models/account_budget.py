...
class CrossoveredBudgetLines(models.Model):
    ...
    p_analytic_account_id = fields.Many2one('purchase.order')

    def _compute_engage_amount(self):
        for line in self:
            acc_ids = tuple(line.general_budget_id.account_ids.ids)
            date_to = line.date_to
            date_from = line.date_from
            if len(acc_ids) == 1:
                query = "SELECT SUM(price_subtotal)\
                        FROM purchase_order_line\
                        WHERE account_analytic_id=%s\
                            AND (DATE(date_planned) BETWEEN '%s' AND '%s')\
                            AND account_id in (%s) AND (state='purchase' OR state='done')" % (line.analytic_account_id.id, date_from, date_to, acc_ids[0])
            else:
                query = "SELECT SUM(price_subtotal)\
                        FROM purchase_order_line\
                        WHERE account_analytic_id=%s\
                            AND (DATE(date_planned) BETWEEN '%s' AND '%s')\
                            AND account_id in %s AND (state='purchase' OR state='done')" % (line.analytic_account_id.id, date_from, date_to, acc_ids)
    ...
    def _compute_percentage(self):
        for line in self:
            if line.planned_amount != 0.00:
                line.percentage = float((line.engage_amount or 0.0) / line.planned_amount) * 100
            else:
...
class AccountBudgetLine(models.Model):
    ...
    tag_ids = fields.Many2many('account.analytic.tag', 'account_budget_line_tag_rel', 'line_id', 'tag_id', string='Tags', copy=True)
