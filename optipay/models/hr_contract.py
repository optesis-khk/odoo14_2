...
class HrContractBonus(models.Model):
    ...
    type_id = fields.Many2one('hr.payroll.structure.type')
    ...
    nbj_aquis = fields.Float("Nombre jours acquis", related='employee_id.nbj_aquis')
   
    
    
    nbj_priss =  fields.Float("Nombre jours pris avec signe négative ",related='employee_id.nbj_pris') 
    
 
    nbj_pris = fields.Float("Nombre de jour pris", default="0", compute='onchange_holiday_tracking', store=True)
    
    #nbj_pris =  fields.Float("Nombre jours pris", compute="_compute_number_pris_new")  

    @api.onchange(nbj_priss, nbj_aquis)
    def _compute_number_pris_new(self): 
        for employee in self:
            employee.nbj_pris = - (employee.nbj_priss) 
            
            
    nbj_restant = fields.Float("Nombre jours restant", compute='_compute_number_restant')  

    @api.onchange(nbj_pris, nbj_aquis)
    def _compute_number_restant(self):
        for employee in self:
            employee.nbj_restant = (employee.nbj_aquis) - (employee.nbj_pris) 
    #fin diw 
