...
class ProvisionRetraiteRuleInput(models.Model):
    ...
    #date debut contrat
    date_start_contrat = fields.Date(related="employee_id.date_start_contrat", string="Date debut contrat")

    duration = fields.Float(default=365, help="Duration dans 1ans")

    date_start_1y_after = fields.Date(compute="_get_compute_date", string="1 ans after Date_start")

    @api.depends('duration', 'date_start_contrat')
    def _get_compute_date(self):
        for r in self:
            if not (r.date_start_contrat and r.duration):
                r.date_start_1y_after = r.date_start_contrat
                continue
            duration = timedelta(days = r.duration, seconds=-1)
            r.date_start_1y_after = r.date_start_contrat + duration


    dateoftoday = fields.Date('Date mise à jour', required=False, readonly=False, select=True, compute='_updatedate')

    @api.onchange('dateoftoday')
    def _updatedate(self):
        for r in self:
            r.dateoftoday = fields.Date.to_string(date.today())     
